    ...
    return (
      ...
      {this.state.cart.map(course => (
                    ...
                    <Card.Body className="cart-body">
                      <Row>
                        ...
                        <Col xs={3} className="body-cmd">
                          <span>
                            <a style={{color:'blue', cursor:'pointer'}} onClick={() => this.handleRemove(course.id_user, course.id_course)}>Remove</a>
                            
                          </span>

                          <div>
                            <a style={{color:'blue', cursor:'pointer'}} onClick={() => this.handleMoveWishlist(course.id_user, course.id_course)}>Move to Wishlist</a>
                          
                          </div>
                        </Col>
                        <Col xs={2} className="price">
                          <b>Rp  {course.price.toLocaleString(navigator.language, { minimumFractionDigits: 0 })}</b>
                        </Col>
                      </Row>
                    </Card.Body>
                  </Card>
            ...
            <Col md={3} className="cart-checkout">
              <h5>Total</h5>
              <h3>Rp.{total.toLocaleString(navigator.language, { minimumFractionDigits: 0 })}</h3>
              {/* <ReactToPrint
                trigger={() => (
